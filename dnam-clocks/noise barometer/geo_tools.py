...
__author__ = "Joshua Blanchard"
__email__ = "jeblanchard@berkeley.edu"
__maintainer__ = "Joshua Blanchard"
...
def __matrix_to_df(file_path, use= "series()", GSE= ""):
    ...
    if use == "series()":
        ...
        if os.path.exists(series_path):
            ...
            df = pd.read_csv(file_path, header= start_row, sep= "\t", low_memory= True, nrows= num_rows)
...
def info(GSE_family, sample_id, info= "age", parent = ""):
    ...
    info_df = __matrix_to_df(parent + "data_GSE/" + GSE_family + "/" + GSE_family + "_series_matrix.txt", use= "info()", GSE= GSE_family)
    ...
    if info == "age":
        ...
        for row in info_series:
            match_down = re.search(r"disease state", row)
            if match_down:
                # let's grab the arthritis status
                match_down_status = re.search(r"(disease state: )(\S+)", row)
                if match_down_status.group(2) == "healthy":
                    return "healthy"
                else:
                    return "down"

    elif info == "depression":

        for row in info_series:
            match_dep = re.search(r"diagnosis", row)
            if match_dep:
                # let's grab the arthritis status
                match_dep_status = re.search(r"(diagnosis: )(\S+)", row)
                if match_arth_status.group(2) == "case":
                    return "case"
                else:
                    return "control"
    else:
...
def process_data(GSE_list, info_= "age"):
    ...
    for row in X_df.index:
        y.append(info(row[0], row[1], info= info_))
